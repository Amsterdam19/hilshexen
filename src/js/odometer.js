...
function animationOdometerInit() {
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          ...
          entry.target.innerHTML = Number(entry.target.getAttribute('data-target'));
  ...
  const hiddenElements = document.querySelectorAll(".odometer");
  hiddenElements.forEach((e) => odometerObserver.observe(e));
}
document.addEventListener("astro:page-load", animationOdometerInit);
document.addEventListener("astro:after-swap", animationOdometerInit);